...
public class UIInput : MonoBehaviour
{
	...
	/// <summary>
	/// Type of _messageViewData expected by the input field.
	/// </summary>
	...
	/// <summary>
	/// What kind of validation to use with the input field's _messageViewData.
	/// </summary>
	...
	/// <summary>
	/// Event delegates triggered when the input field submits its _messageViewData.
	/// </summary>
	...
	// MS_ADD
	public bool DoNotSelectAllOnEvent = false;
	...
	protected virtual void Update ()
	{
		...
		if (mSelectMe != -1 && mSelectMe != Time.frameCount)
		{
...
#endif // MOBILE
			{
				...
				if (!DoNotSelectAllOnEvent) { // MS_ADD
					mSelectionStart = 0;
					mSelectionEnd = string.IsNullOrEmpty(mValue) ? 0 : mValue.Length;
				}
				mDrawStart = 0;
	...
	public virtual bool ProcessEvent (Event ev)
	{
		...
		// TODO: @Baz This is a temporary fix. Detected on Unity 5.1.3, possibly fixed in recent versions.
		//       OSX build doesn't detect LeftArrow, RightArrow and Shift(-s) in UnityEngine.Event correctly. 
#if UNITY_STANDALONE_OSX
		if (isMac && Event.current != null) {
			var currentEventCharacter = (int)Event.current.character;
			System.Func<bool> hackShift = () => {
				return Input.GetKey(KeyCode.LeftShift) || Input.GetKey(KeyCode.RightShift);	
			};
			
			switch (currentEventCharacter) {
				case 28:
