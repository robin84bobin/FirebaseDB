...
[ CustomEditor ( typeof ( MSScrollViewDataSource ) ) ]
public abstract class IPPickerInspector : Editor 
{
	...
	protected void DrawResetButton ()
	{
		...
		{
			MSScrollViewDataSource picker = (MSScrollViewDataSource)target;
			
			if ( picker.scrollView == null )
				picker.scrollView = picker.gameObject.GetComponentInChildren(typeof(IPCycler)) as IPCycler;

			picker.scrollView.EditorInit();
			picker.scrollView.SendMessageUpwards("RebuildWidgets", SendMessageOptions.DontRequireReceiver); //sendmessage upwards from cycler in case there are multiple pickers
		}
	}
